...
import com.maxtechnologies.cryptomax.ChartActivites.ChartActivity;
import com.maxtechnologies.cryptomax.Controllers.AlertController;
import com.maxtechnologies.cryptomax.Controllers.ChartController;
...
public class Chart extends CombinedChart {
    ...
    public void setData(final boolean reFocus) {
        ...
        sigCandles = OtherTools.getSigCandles(widthSec, candles.size() + tickerLineLen, tempTime);

        if (Settings.smaChecked) {
            for (int i = 0; i < 3; i++) {
                if (Settings.sma[i] > 0) {
                    float[] sma = ChartController.getSMA(candles, Settings.sma[i]);
        ...
        if (Settings.emaChecked) {
            for (int i = 0; i < 3; i++) {
                if (Settings.ema[i] > 0) {
                    float[] ema = ChartController.getEMA(candles, Settings.ema[i]);
        ...
        if (Settings.bolChecked) {
            if (Settings.bol[0] > 0 && Settings.bol[1] > 0) {
                float[][] bollinger = ChartController.getBollinger(candles, Settings.bol[0], Settings.bol[1]);
        ...
        if (Settings.sarChecked) {
            if (Settings.sar[0] > 0 && Settings.sar[1] > 0) {
                float[] sar = ChartController.getSAR(candles, Settings.sar[0], Settings.sar[1]);
                addSAR(sar);
            }
        }

        if (Settings.rsiChecked) {
            if (Settings.rsi[0] > 0 && Settings.rsi[1] > 0) {
                float[] rsiArr = ChartController.getRSI(candles, Settings.rsi[0]);

                rsi = new ArrayList<>();
                for (int i = 0; i < rsiArr.length; i++) {
                    rsi.add(rsiArr[i]);
                }
                addRSI();
            }
    ...
    public void updateData(float price) {
        if (getCandleData() != null && getLineData()!= null) {
            ...
            if (Settings.rsiChecked) {
                if (Settings.rsi[0] > 0 && Settings.rsi[1] > 0) {
                    float[] rsi = ChartController.getRSI(candles, Settings.rsi[0]);
                    ArrayList<Entry> entries = scaleRSI();

                    ILineDataSet lineSet = data.getLineData().getDataSetByLabel("RSI", false);
                    lineSet.getEntryForIndex(lineSet.getEntryCount() - 1).setY(entries.get(entries.size() - 1).getY());
                }
            }
    ...
    private void scaleUnderlays() {
        if (Settings.rsiChecked) {
            if (Settings.rsi[0] > 0 && Settings.rsi[1] > 0) {
                ArrayList<Entry> entries = scaleRSI();
                ILineDataSet lineSet = data.getLineData().getDataSetByLabel("RSI", false);
                int length = lineSet.getEntryCount();
                for (int i = 0; i < length; i++) {
                    lineSet.removeLast();
                }
                for (int i = 0; i < entries.size(); i++) {
                    lineSet.addEntry(entries.get(i));
                }
    ...
    private class DateAxisFormatter implements IAxisValueFormatter {
        ...
        public String getFormattedValue(float value, AxisBase axis) {
            ...
            return OtherTools.getChartDateString(time, widthSec, sigIndex);
        }
    }
