...
import com.ripple.TestInterface;
...
//        return fields.keySet().stream().sorted().iterator();
    ...
    protected Hash256 signingHash(HashPrefix txSign) {
        ...
        toBytesSink(signing, new TestInterface() {
            @Override
    ...
    protected byte[] signingData(HashPrefix txSign) {
        ...
        bl.add(txSign.bytes());
        toBytesSink(bl, new TestInterface() {
            @Override
            public boolean test(Field field) {
                return field.isSigningField();
            }
    ...
    public void toBytesSink(BytesSink to, TestInterface p) {
    ...
    public void toBytesSink(BytesSink to) {
        toBytesSink(to, new TestInterface() {
            @Override
            public boolean test(Field field) {
