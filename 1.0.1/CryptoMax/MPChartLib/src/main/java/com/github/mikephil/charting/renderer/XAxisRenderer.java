...
public class XAxisRenderer extends AxisRenderer {
    ...
    protected void computeAxisValues(float min, float max) {
        if(mXAxis.labelPositions == null || mXAxis.labelPositions.length < 2) {
            ...
            for(int i = 0; i < mXAxis.labelPositions.length; i++) {
                float position = mXAxis.labelPositions[i];
                if(position > min && position < max) {
                    positions.add(position);
                }
            }

            mXAxis.mEntries = new float[positions.size()];
            for(int i = 0; i < positions.size(); i++) {
                mXAxis.mEntries[i] = positions.get(i);
            }
            mXAxis.mEntryCount = mXAxis.mEntries.length;

            double interval = mXAxis.labelPositions[1] - mXAxis.labelPositions[0];
            ...
            if (mAxis.isCenterAxisLabelsEnabled()) {
                ...
                if (mAxis.mCenteredEntries.length < mXAxis.mEntries.length) {
                    mAxis.mCenteredEntries = new float[mXAxis.mEntries.length];
                }
