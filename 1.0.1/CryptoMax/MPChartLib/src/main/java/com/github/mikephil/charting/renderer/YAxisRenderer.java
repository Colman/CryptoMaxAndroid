...
public class YAxisRenderer extends AxisRenderer {
    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        ...
        MPPointD pixelPoint = mTrans.getPixelForValues(0, mYAxis.getExtraYValue());
        float extraYPos = (float) pixelPoint.y + offset;
        float textHeight = Utils.calcTextHeight(mAxisLabelPaint, "A");
        for (int i = from; i < to; i++) {
            float yPos = positions[i * 2 + 1] + offset;

            if(yPos - textHeight > extraYPos || yPos + textHeight < extraYPos) {
                String text = mYAxis.getFormattedLabel(i);
                c.drawText(text, fixedPosition, yPos, mAxisLabelPaint);
            }
        }

        MPPointD topLeft = mTrans.getValuesByTouchPoint(mViewPortHandler.contentLeft(), mViewPortHandler.contentTop());
        MPPointD bottomRight = mTrans.getValuesByTouchPoint(mViewPortHandler.contentRight(), mViewPortHandler.contentBottom());
        if(mYAxis.getExtraYValue() < topLeft.y && mYAxis.getExtraYValue() > bottomRight.y) {
            Paint extraPaint = mAxisLabelPaint;
            extraPaint.setTextSize(extraPaint.getTextSize() * 1.02f);
            Typeface typeFace = Typeface.create(extraPaint.getTypeface(), Typeface.BOLD);
            extraPaint.setTypeface(typeFace);
            String text = mYAxis.getValueFormatter().getFormattedValue(mYAxis.getExtraYValue(), mYAxis);
            c.drawText(text, fixedPosition, extraYPos, extraPaint);
        }
    }
